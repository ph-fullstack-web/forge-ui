...
import {getActivityListAsDropdown} from 'lib/helpers';
import {CECRequest, Activity, CECRequestAttachment} from 'models/core';
...
interface CECRequestEditFormProps {
...
export const CECRequestEditForm = ({
  cecRequest,
  activityList,
  approverName,
  onUpdateCECRequest,
  ...
  const formik = useFormik<CECRequest>({
    initialValues: {
      ...
      dateStarted: cecRequest.dateStarted,
  ...
  const handleOnDrop = (uploadedFiles: File[]) => {
    setNewAttachments(uploadedFiles);
  };

  const handleRemoveFile = (index: number) => {
    const newNewAttachments = Array.from(newAttachments);
    newNewAttachments.splice(index, 1);
    setNewAttachments(newNewAttachments);
  };

  const handleRemoveExistingFile = (index: number) => {
    const fileToBeDeleted = {...values.attachments[index]};
          ...
          <Grid.Col md={12}>
            ...
            >
              <Select
                ...
                searchable
                clearable
                value={values.activity.activityId.toString()}
